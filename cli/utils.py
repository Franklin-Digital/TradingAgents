...
def _llm_provider_table() -> list[tuple[str, str, str | None]]:
    ...
    # Franklin's Bifrost AI Gateway is listed first: it is the production
    # default. Override the endpoint with BIFROST_BASE_URL (or the legacy
    # VLLM_BASE_URL) when running against a different deployment.
    bifrost_url = os.environ.get("BIFROST_BASE_URL") or os.environ.get(
        "VLLM_BASE_URL", "https://ai-gateway.franklinfinancial.ai/v1"
    )
    return [
        ("vLLM (Franklin AI Gateway / Bifrost)", "vllm", bifrost_url),
