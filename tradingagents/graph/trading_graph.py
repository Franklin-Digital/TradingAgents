...
class TradingAgentsGraph:
    ...
    def _fetch_returns(
        ...
        try:
            ...
            # Franklin routes the realized-return lookup through the licensed
            ...
            if stock_start == 0 or bench_start == 0:
                # A zero open price is a vendor sentinel, not a real quote:
                # dividing by it would fabricate an infinite return.
                return None, None, None, None

            raw = float((stock["Close"].iloc[holding_days] - stock_start) / stock_start)
            bench_ret = float((bench["Close"].iloc[holding_days] - bench_start) / bench_start)
            alpha = raw - bench_ret
            ...
            # _parse_ohlcv_csv returns a Date/Datetime column, not a DatetimeIndex.
            _date_col = "Datetime" if "Datetime" in stock.columns else "Date"
            resolution_date = stock[_date_col].iloc[holding_days].strftime("%Y-%m-%d")
