...
OPENAI_COMPATIBLE_PROVIDERS: dict[str, ProviderSpec] = {
    ...
    # Franklin AI Gateway (Bifrost) — runs on mac-pro, OpenAI Chat Completions
    # compatible. Routes to OpenRouter (DeepSeek) today; historically vLLM on
    # the DGX, hence the provider name. No API key required on a private
    # gateway, so the key is optional and an "EMPTY" placeholder is sent.
    # BIFROST_BASE_URL (or the legacy VLLM_BASE_URL) overrides the endpoint.
    "vllm": ProviderSpec(
        base_url=os.getenv(
            "BIFROST_BASE_URL", os.getenv("VLLM_BASE_URL", "http://localhost:8080/v1")
        ),
        base_url_env="BIFROST_BASE_URL",
        chat_class=VLLMChatOpenAI,
        key_optional=True,
    ),
...
class OpenAIClient(BaseLLMClient):
    ...
    def get_llm(self) -> Any:
        ...
        if spec is not None:
            ...
            # Cloudflare Access service token — required when the Bifrost
            # gateway is fronted by a CF Access policy. Only set when both
            # halves of the service token are present.
